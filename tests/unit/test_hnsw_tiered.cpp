...
template <typename index_type_t>
class SQ8TieredHNSWTest : public ::testing::Test {
public:
    using data_t = typename index_type_t::data_t;
    using dist_t = typename index_type_t::dist_t;
...
protected:
    ...
    CreateSQ8TieredIndex(tieredIndexMock &mock_thread_pool, size_t dim = 16,
                         VecSimMetric metric = VecSimMetric_L2, size_t normSetSize = 100,
    ...
    // --- Accessor helpers (SQ8TieredHNSWTest is a friend of TieredHNSWIndex) ---
    ...
    void GenerateVectorData(data_t *output, size_t dim, float base_value) {
        for (size_t i = 0; i < dim; i++) {
            float val = base_value + static_cast<float>(i) * 0.1f;
...
// Type list: FP32 single, FP32 multi, FP16 single, FP16 multi
using SQ8DataTypeSet = ::testing::Types<SQ8IndexType<VecSimType_FLOAT32, false, float>,
                                        SQ8IndexType<VecSimType_FLOAT32, true, float>,
                                        SQ8IndexType<VecSimType_FLOAT16, false, float16, float>,
                                        SQ8IndexType<VecSimType_FLOAT16, true, float16, float>>;

TYPED_TEST_SUITE(SQ8TieredHNSWTest, SQ8DataTypeSet);

// -------------------------------------------------------------------
// Accumulation Phase Core Tests
// -------------------------------------------------------------------

TYPED_TEST(SQ8TieredHNSWTest, AccumulationPhaseInitialization) {
    // Verify that creating an SQ8 tiered index enters accumulation phase.
    size_t dim = 16;
    size_t normSetSize = 100;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
        this->CreateSQ8TieredIndex(mock_thread_pool, dim, VecSimMetric_L2, normSetSize);
...
TYPED_TEST(SQ8TieredHNSWTest, RunningSumAccuracy) {
...
TYPED_TEST(SQ8TieredHNSWTest, QueryDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, RangeQueryDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, AddVectorDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, DeleteVectorDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, OverwriteDuringAccumulation) {
    // Vector overwrite should update running sum correctly (only for single-label).
    if (TypeParam::isMulti()) {
        GTEST_SKIP() << "Overwrite test is for single-label only";
    }
...
TYPED_TEST(SQ8TieredHNSWTest, BackendCreatedAtThreshold) {
...
TYPED_TEST(SQ8TieredHNSWTest, MeanComputedCorrectly) {
...
TYPED_TEST(SQ8TieredHNSWTest, TransitionOutOfAccumulation) {
    // Verify state transition flags.
    size_t dim = 4;
...
TYPED_TEST(SQ8TieredHNSWTest, PendingJobsAfterTransition) {
    // After transition, pending insert jobs exist in labelToInsertJobs.
    size_t dim = 4;
    size_t normSetSize = 5;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
    ...
    // Flat buffer should still hold all vectors.
    ASSERT_EQ(this->getFrontendIndex(tiered_index)->indexSize(), normSetSize);
}

// -------------------------------------------------------------------
// Post-Accumulation Operations Tests
// -------------------------------------------------------------------

TYPED_TEST(SQ8TieredHNSWTest, QueryAfterAccumulation) {
    // After accumulation, queries should merge flat + backend results.
    size_t dim = 8;
    size_t normSetSize = 10;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
    ...
    // Execute all pending insert jobs to move vectors to backend.
    while (!mock_thread_pool.jobQ.empty()) {
        mock_thread_pool.thread_iteration();
    }

    // Add more vectors after transition (these go through normal path).
    for (size_t i = normSetSize; i < normSetSize + 5; i++) {
        TEST_DATA_T vec[dim];
        this->GenerateVectorData(vec, dim, static_cast<float>(i));
        VecSimIndex_AddVector(tiered_index, vec, i);
    }

    // Execute new insert jobs.
    while (!mock_thread_pool.jobQ.empty()) {
        mock_thread_pool.thread_iteration();
    }

    // Query should find results from the backend.
    TEST_DATA_T query[dim];
    this->GenerateVectorData(query, dim, 0.0f);
    auto *results = VecSimIndex_TopKQuery(tiered_index, query, 5, nullptr, BY_SCORE);
    ASSERT_NE(results, nullptr);
...
TYPED_TEST(SQ8TieredHNSWTest, NewVectorsAfterAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, DeleteFromBackendAfterAccumulation) {
...
// -------------------------------------------------------------------
// Accumulation + Concurrent Operations Tests
// -------------------------------------------------------------------

TYPED_TEST(SQ8TieredHNSWTest, ParallelInsertDuringAccumulation) {
    // Multiple threads adding vectors during accumulation phase.
    size_t dim = 8;
    size_t normSetSize = 1000; // High threshold so accumulation persists.
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
        this->CreateSQ8TieredIndex(mock_thread_pool, dim, VecSimMetric_L2, normSetSize);

    size_t n_per_thread = 50;
    size_t n_threads = 4;

    auto insert_fn = [&](size_t thread_id) {
        for (size_t i = 0; i < n_per_thread; i++) {
            TEST_DATA_T vec[dim];
            float val = static_cast<float>(thread_id * n_per_thread + i);
            this->GenerateVectorData(vec, dim, val);
            labelType label = thread_id * n_per_thread + i;
            VecSimIndex_AddVector(tiered_index, vec, label);
        }
    };

    std::vector<std::thread> threads;
    for (size_t t = 0; t < n_threads; t++) {
        threads.emplace_back(insert_fn, t);
    }
    for (auto &t : threads) {
        t.join();
    }
    ...
    ASSERT_TRUE(this->getIsInAccumulationPhase(tiered_index));
    ASSERT_EQ(this->getFrontendIndex(tiered_index)->indexSize(), n_per_thread * n_threads);
}

TYPED_TEST(SQ8TieredHNSWTest, ConcurrentSearchDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, IndexSizeDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, CapacityDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, ZeroAccumulationThreshold) {
    // QuantNormalizationSetSize=0 should skip accumulation phase entirely.
    size_t dim = 4;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
        this->CreateSQ8TieredIndex(mock_thread_pool, dim, VecSimMetric_L2, 0 /* normSetSize=0 */);
...
TYPED_TEST(SQ8TieredHNSWTest, SingleVectorAccumulation) {
    // Accumulation with threshold=1: immediately transition after first insert.
    size_t dim = 4;
    size_t normSetSize = 1;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
        this->CreateSQ8TieredIndex(mock_thread_pool, dim, VecSimMetric_L2, normSetSize);
    ...
    TEST_DATA_T vec[dim];
    this->GenerateVectorData(vec, dim, 2.0f);
    VecSimIndex_AddVector(tiered_index, vec, 0);

    // Should have transitioned after first vector.
    ASSERT_FALSE(this->getIsInAccumulationPhase(tiered_index));
    ASSERT_EQ(this->getFrontendIndex(tiered_index)->indexSize(), 1);
}

TYPED_TEST(SQ8TieredHNSWTest, AllVectorsDeletedBeforeThreshold) {
...
TYPED_TEST(SQ8TieredHNSWTest, ExactlyAtThreshold) {
    // Backend created exactly when flat buffer reaches quantNormalizationSetSize.
    size_t dim = 4;
    size_t normSetSize = 3;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
    ...
    // Add exactly normSetSize vectors.
    for (size_t i = 0; i < normSetSize; i++) {
        TEST_DATA_T vec[dim];
        this->GenerateVectorData(vec, dim, static_cast<float>(i + 1));
        VecSimIndex_AddVector(tiered_index, vec, i);
...
// -------------------------------------------------------------------
// Distance Metrics with Accumulation
// -------------------------------------------------------------------

TYPED_TEST(SQ8TieredHNSWTest, AccumulationWithCosine) {
    // Accumulation with Cosine metric.
    size_t dim = 8;
    size_t normSetSize = 5;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
    ...
    // Execute jobs and verify search works.
    while (!mock_thread_pool.jobQ.empty()) {
        mock_thread_pool.thread_iteration();
    }
    ...
    TEST_DATA_T query[dim];
    this->GenerateVectorData(query, dim, 1.0f);
    auto *results = VecSimIndex_TopKQuery(tiered_index, query, 3, nullptr, BY_SCORE);
    ASSERT_NE(results, nullptr);
    ASSERT_GT(VecSimQueryReply_Len(results), 0);
    VecSimQueryReply_Free(results);
}

TYPED_TEST(SQ8TieredHNSWTest, AccumulationWithIP) {
    // Accumulation with Inner Product metric.
    size_t dim = 8;
    size_t normSetSize = 5;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
...
TYPED_TEST(SQ8TieredHNSWTest, AccumulationMultiLabel) {
    // Multi-label: multiple vectors per label during accumulation.
    if (!TypeParam::isMulti()) {
        GTEST_SKIP() << "Multi-label test skipped for single-label index";
    }
...
TYPED_TEST(SQ8TieredHNSWTest, DeleteMultiLabelDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, BatchIteratorDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, DebugInfoDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, WriteInPlaceDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, WriteInPlaceAfterAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, BufferLimitWithAccumulation) {
    ...
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index = this->CreateSQ8TieredIndex(mock_thread_pool, dim, VecSimMetric_L2,
...
TYPED_TEST(SQ8TieredHNSWTest, QuantizedSearchQuality) {
...
TYPED_TEST(SQ8TieredHNSWTest, FullFlowAsyncInsertAndSearch) {
...
TYPED_TEST(SQ8TieredHNSWTest, FullFlowWithDeletions) {
    // Accumulation -> transition -> insert -> delete -> verify correctness.
    size_t dim = 4;
    size_t normSetSize = 10;
    auto mock_thread_pool = tieredIndexMock();
    auto *tiered_index =
    ...
    // Search should return remaining vectors.
    TEST_DATA_T query[dim];
    this->GenerateVectorData(query, dim, static_cast<float>(normSetSize - 1));
    auto *results = VecSimIndex_TopKQuery(tiered_index, query, normSetSize, nullptr, BY_SCORE);
    ASSERT_NE(results, nullptr);
    // Should get at most normSetSize/2 results (the non-deleted ones).
    ASSERT_LE(VecSimQueryReply_Len(results), normSetSize);
    ASSERT_GE(VecSimQueryReply_Len(results), normSetSize / 2);
    VecSimQueryReply_Free(results);
}

TYPED_TEST(SQ8TieredHNSWTest, DeleteThenReinsertDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, RunningSumPrecision) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineAccumulationPhase) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineTransitionAndQuery) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineOverwriteDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineDeleteDuringAccumulation) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineSearchAccuracyAfterTransition) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineRangeQueryAfterTransition) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineMeanCorrectness) {
...
TYPED_TEST(SQ8TieredHNSWTest, CosineDeleteAndReinsertAfterTransition) {
