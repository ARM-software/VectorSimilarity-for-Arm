...
// ============================================================
// Tests for DistanceCalculatorWithNorm
// ============================================================
...
static void buildStorageBlob(const std::shared_ptr<VecSimAllocator> &allocator, const float *x,
    ...
    if (metric == VecSimMetric_IP) {
        ...
        pp->preprocessForStorage(x, storage_blob, sz);
        delete pp;
    } else {
        auto *pp = new (allocator)
...
// Helper: build query blob (FP32) from original vector y and mean.
static void buildQueryBlobFP32(const std::shared_ptr<VecSimAllocator> &allocator, const float *y,
                               const float *mean, size_t dim, VecSimMetric metric,
                               void *&query_blob) {
    vecsim_stl::vector<float> mean_vec(allocator);
    for (size_t i = 0; i < dim; ++i)
...
TEST(DistanceCalculatorWithNormTest, CalcDistanceForQuery_IP_FP32) {
    ...
    buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_IP, query_blob);

    auto asym_func = IP_SQ8_FP32_GetDistFunc(dim);
    auto sym_func = IP_SQ8_SQ8_GetDistFunc(dim);
...
TEST(DistanceCalculatorWithNormTest, CalcDistanceForQuery_L2_FP32) {
    ...
    buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_L2, query_blob);

    auto asym_func = L2_SQ8_FP32_GetDistFunc(dim);
    auto sym_func = L2_SQ8_SQ8_GetDistFunc(dim);
...
TEST(DistanceCalculatorWithNormTest, CalcDistanceForQuery_FP16) {
    ...
    pp_ip->preprocessForStorage(x, storage_blob, sz);
    delete pp_ip;
    ...
    auto asym_func = IP_SQ8_FP16_GetDistFunc(dim);
    auto sym_func = IP_SQ8_SQ8_GetDistFunc(dim);
    ...
    // FP16 has more quantization error
    EXPECT_NEAR(got, expected, 0.1f) << "Asymmetric IP FP16 distance mismatch";
...
TEST(DistanceCalculatorWithNormTest, ZeroMean_MatchesBaseSQ8) {
    ...
    pp_ip->preprocessForStorage(x, x_norm_blob, sx);
    ...
    auto *pp_ip2 =
        new (allocator) QuantPreprocessor<float, VecSimMetric_IP, true>(allocator, dim, zero_mean);
    pp_ip2->preprocessForStorage(y, y_norm_blob, sx);
    delete pp_ip2;
...
TEST(DistanceCalculatorWithNormTest, SymmetricVsAsymmetric_Sanity) {
    ...
    std::shared_ptr<VecSimAllocator> allocator = VecSimAllocator::newVecsimAllocator();
    constexpr size_t dim = 8;
    float x[dim] = {3.0f, 1.0f, 4.0f, 1.0f, 5.0f, 9.0f, 2.0f, 6.0f};
    float y[dim] = {2.0f, 7.0f, 1.0f, 8.0f, 2.0f, 8.0f, 1.0f, 8.0f};
    ...
    // IP
    {
        ...
        buildStorageBlob(allocator, x, mean, dim, VecSimMetric_IP, x_blob);
        buildStorageBlob(allocator, y, mean, dim, VecSimMetric_IP, y_blob);
        buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_IP, y_query);
        ...
        EXPECT_NEAR(sym_dist, bf, std::abs(bf) * 0.005f + 0.1f) << "Symmetric IP vs brute-force";
        EXPECT_NEAR(asym_dist, bf, std::abs(bf) * 0.005f + 0.1f) << "Asymmetric IP vs brute-force";
    ...
    // L2
    {
        ...
        buildStorageBlob(allocator, x, mean, dim, VecSimMetric_L2, x_blob);
        buildStorageBlob(allocator, y, mean, dim, VecSimMetric_L2, y_blob);
        buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_L2, y_query);
        ...
        EXPECT_NEAR(sym_dist, bf, 0.5f) << "Symmetric L2 vs brute-force";
        EXPECT_NEAR(asym_dist, bf, 0.5f) << "Asymmetric L2 vs brute-force";
...
TEST(DistanceCalculatorWithNormTest, RandomVectors_SignIndexErrors) {
    // Generate random vector pairs, compute WithNorm distances and verify against brute-force.
    // This catches sign/index-offset bugs in the correction terms.
    std::shared_ptr<VecSimAllocator> allocator = VecSimAllocator::newVecsimAllocator();
    ...
    auto asym_ip = IP_SQ8_FP32_GetDistFunc(dim);
    auto sym_ip = IP_SQ8_SQ8_GetDistFunc(dim);
    auto asym_l2 = L2_SQ8_FP32_GetDistFunc(dim);
    auto sym_l2 = L2_SQ8_SQ8_GetDistFunc(dim);
    ...
    for (int trial = 0; trial < 100; ++trial) {
        ...
        buildStorageBlob(allocator, x, mean, dim, VecSimMetric_IP, x_blob);
        buildStorageBlob(allocator, y, mean, dim, VecSimMetric_IP, y_blob);
        buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_IP, y_query_ip);
        ...
        buildQueryBlobFP32(allocator, y, mean, dim, VecSimMetric_L2, y_query_l2);
        ...
        if (std::abs(got_ip_sym - bf_ip) > 0.1f)
            ++failures;
        if (std::abs(got_l2_asym - bf_l2) > 0.5f)
            ++failures;
        if (std::abs(got_l2_sym - bf_l2) > 0.5f)
            ++failures;
